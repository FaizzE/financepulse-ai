...
export async function categorizeExpense(description: string): Promise<string> {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expense categorization AI. Categorize a single expense into EXACTLY ONE of these categories: Food, Transportation, Entertainment, Shopping, Bills, Healthcare, Other. Respond ONLY with that single category word.",
        },
        {
          role: "user",
    ...
    // ✅ Perfect match
    const exact = validCategories.find((cat) => cleaned === cat);
    if (exact) return capitalize(exact);
    ...
    console.log("⚠️ Fallback to Other");
    return "Other";
  } catch (err) {
    console.error("❌ categorizeExpense error:", err);
    return "Other";
  }
}
